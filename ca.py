...
# Function to record employee entry time in the database
def record_entry_time(employee_id):
    # Connect to MySQL database
    mydb = mysql.connector.connect(
      host="localhost",
      user="root",
    ...
    sql = "INSERT INTO entry_records (userid, entry_time) VALUES (%s, %s)"
    val = (employee_id, datetime.now())
    mycursor.execute(sql, val)
